...
namespace bt 
{
  ...
  //the given key.
  uint32_t btree::bsearch_key(bt_node *n, uint32_t key) 
  {
  ...
  bool btree::search_kv(uint32_t key, uint32_t &value)
  {
    ...
    while(temp) 
    { 
      uint32_t idx = bsearch_key(temp, key);
      if(temp->is_leaf && key == temp->values[idx].first) 
      {
  ...
  bool btree::delete_kv(uint32_t key) {
    ...
    while(temp != root || temp->values.size() < DEG/2) 
    {
      ...
      switch(mode)
      {
        //Stealing from sibblings
        case STEAL:
          uint32_t iter;

          if(temp->is_leaf) 
          {
            //Stealing from left sibbling
            if(sib_idx < parent_idx) 
            {
                for(iter=sibbling->values.size()-1; 
                    iter>=sibbling->values.size()-req_size; iter--) 
                {
                    temp->values.insert(temp->values.begin(), 
                               std::move(sibbling.values[iter]));
                    temp->ptrs.push_back(NULL);          
                    temp->values.size()++;
                }
                sibbling->values.erase(sibbling->values.begin()+(sibbling->values.size()-req_size), 
                                       sibbling->values.end());
                sibbling->ptrs.erase(sibbling->ptrs.begin()+(sibbling->values.size()-req_size)+1, 
                                       sibbling->ptrs.end());
            } 
            else //Stealing from right sibbling
            {
                ...
                sibbling->values.erase(sibbling->values.begin(), 
                                       sibbling->values.begin()+req_size);
                sibbling->ptrs.erase(sibbling->ptrs.begin(),
                                      sibbling->ptrs.begin()+req_size);
            }

            //Update sibbling current size
            sibbling->values.size() -= req_size;

            //Update the parent node with the correct key after stealing
            parent->values[parent_idx].first = temp->values[0].first;
            parent->values[parent_idx].second = temp->values[0].second;
          }
          else
          {
            if(sib_idx < parent_idx) 
            {
                  ...
                  iter>=sibbling->values.size()-req_size; iter--)
              {
                  ...
                  parent->values[parent_idx-1] = std::move(sibbling->values[iter]);

                temp->values.size()++;
                sibbling->values.size()--;
              }
            }
            else
            {
              for(iter=0;iter < req_size; iter++)
              {
                ...
                temp->ptrs.insert(temp->ptrs.end(),
                                  std::move(sibbling->ptr[iter]));
                parent->values[parent_idx] = std::move(sibbling->values[iter]);
                temp->values.size()++;
                sibbling->values.size()--;
              }
        ...
        case MERGE:
          ...
          for(iter=0; iter<from->values.size(); ++iter) 
          {
            to->values.insert(to->values.end(), 
                              std::move(from->values[iter]));
            to->ptrs.insert(to->ptrs.end(), NULL);
          }
          
          if(sib_idx < parent_idx) 
          {
            parent->values.erase(parent->values.begin()+parent_idx-1);
            delete from;
            parent->ptrs.erase(parent->ptrs.begin()+parent_idx);
          }
          else
          {
            parent->values.erase(parent->values.begin()+sib_idx-1);
            delete from;
            parent->ptrs.erase(parent->ptrs.begin()+sib_idx);
          }
